...
if __name__ == '__main__':
    ...
    # the first portion will play back between times 2 and 3 at a speed of 0.5/s
    t1 = LinearPlaybackDefinedSpeed(t_min,3,0.5)
    
    # the second portion will play back the remainder of the data over 4 seconds
    t2 = LinearPlaybackDefinedDuration(3,t_max,4) 
    
    # combine the two timings into one.
    ...
    def draw_main(t,fig,timing=Timing()):
        ...
        ax.set_xlim([-2,14])
        ax.set_ylim([-2,2])
        ax.set_title('index time = '+'{:02.3f}'.format(t)+'\n'+'playback rate is '+'{:02.3f}'.format(timing.current_playback_rate))
        
        fig.tight_layout()
        return None
    
    scene1 = Scene(draw_main,timing_main)
    
    fig = plt.figure(figsize=(9,6))
    video = Video([title_scene,scene1],fig,fps=12)
